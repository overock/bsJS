	...
	bs.cls( 'Dom', function( fn, bs ){
		...
		html = (function(doc){
			...
			return function( str, target, mode ){
				var t0, t1, t2, t3, t4, t5, i, j, n0, n1, n2, parent;
				...
				t1 = t1.toLowerCase();
				if( mode == 'html' && target.nodeName.toLowerCase() == 'table' && t1 == 'tr' ) t1 = 'tbody';
				if( mode == '>' || 'html+' && t1 == 'tr' ) target = target.getElementsByTagName('tbody')[0] || ( target.appendChild(tbody), target.getElementsByTagName('tbody')[0] );
				if( tags[t1] ){
				...
				if( !target ){
					target = {length : 0};
					while( i-- ) target[target.length++] = parent.childNodes[i];Array.prototype.reverse.call(target);
				}else if( mode == 'html' ){
					bs.Dom(target.childNodes).S(null);
					while( i-- ) target.appendChild(parent.childNodes[0]);
				}else if( mode == '+html' ) while( i-- ) target.insertBefore( parent.childNodes[parent.childNodes.length - 1], target.childNodes[0] );
		...
		fn['html+'] = function( d, v ){return d.nodeName.toLowerCase() == 'table' ? html( v, d, 'html+' ) : html( v, d );},
