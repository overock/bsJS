bs.$register( 'class', 'd3', function( $fn ){
	var ani, key, ANI;
	bs.css( '.PLANE' ).$( 'position', 'absolute'),
	bs.css( '.PLANE img' ).$( 'width', '100%', 'height','100%'),
	ANI = bs.ANI.ani,
	(function(){
		var t0, i;
		t0 = 'x,y,z,width,height,src,world,@'.split(','), key = {}, i = t0.length;
		while( i-- ) key[t0[i]] = 1;
	})(),
	ani = function( $time ){
	...
	$fn.$ = function(){
		...
		if( ( t0 = arguments[0] ).charAt(0) == '@' ){
			if( this[t0] ) t0 = this[t0];
				...
				ANI:ani,
				div:bs.dom('<div class="PLANE"></div>').$('display','none','this'),
